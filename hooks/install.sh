...
if [ ! -f "$PROJECT_ROOT/living-doc-config.yaml" ]; then
    echo "⚠️  Warning: living-doc-config.yaml not found"
...
if [ -f "\$HOOK_SCRIPT" ]; then
    ...
    echo "⚠️  Warning: \$HOOK_SCRIPT not found, skipping doc validation"
    exit 0
fi
EOF
...
if [ -f "\$HOOK_SCRIPT" ]; then
    ...
    echo "⚠️  Warning: \$HOOK_SCRIPT not found, skipping commit message validation"
    exit 0
fi
EOF
...
echo "   • post-commit    - Dashboard updates"
echo ""
echo "📋 Hook locations:"
...
echo ""
echo "💡 Usage tips:"
echo "   • Test hooks:  git commit --dry-run"
